    ...
    for address in address_list:
        ...
        if address and address[0].isdigit() and " " in address:
            try:
                ...
                return (
                    normalized_address.get("address_line_1", None)
                    + " "
                    + normalized_address.get("address_line_2")
                    if normalized_address.get("address_line_2", None)
                    else normalized_address.get("address_line_1", None)
                )
            except Exception:
